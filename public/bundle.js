...
/***/ function(module, exports, __webpack_require__) {
	...
	var tooltip = d3.select('body').append('div').style('opacity', '0').style('position', 'absolute').attr('class', 'toolTip').style('padding', '5px').style('background', 'lightgray');
	fetch('https://raw.githubusercontent.com/FreeCodeCamp/ProjectReferenceData/master/cyclist-data.json').then(function (response) {
	  ...
	  var x = d3.scaleLinear().domain([maxTime - minTime + 15, 0]).range([0, width]);
	  var yAxis = d3.axisLeft(y);
	  var xAxis = d3.axisBottom(x);
	  var chartGroup = svg.append('g').attr('transform', 'translate(' + margin.left + ', ' + margin.top + ')');
	  chartGroup.selectAll('circle').data(data).enter().append('circle').attr('class', 'circle').attr('fill', function (d) {
	    return d.Doping ? 'red' : 'darkgreen';
	  }) // eslint-disable-line
	  ...
	  chartGroup.selectAll('text').data(data).enter().append('text').attr('x', function (d) {
	    return x(d.Seconds - minTime - 3);
	  }).attr('y', function (d) {
	    return y(d.Place + 0.25);
	  }).text(function (d) {
	    return d.Name;
	  });
	  chartGroup.append('g').attr('class', 'axis').call(yAxis);
	  chartGroup.append('g').attr('transform', 'translate(0, ' + height + ')').attr('class', 'axis').call(xAxis);
	  chartGroup.append('text').attr('text-anchor', 'middle').attr('transform', 'translate(' + width / 2 + ', -30)').text('DOPING IN PROFESSIONAL RACING');
	  chartGroup.append('text').attr('text-anchor', 'middle').attr('transform', 'translate(' + width / 2 + ', -10)').text("35 Fastest Times up Alpe d'Huez");
	  chartGroup.append('text').attr('text-anchor', 'middle').attr('transform', 'translate(' + width / 2 + ', ' + (height + margin.top) + ')').text('Seconds Behind Fastest Time');
	  chartGroup.append('text').attr('transform', 'translate(-' + margin.left / 2 + ', ' + height / 2 + ')rotate(-90)').text('Ranking');
	});
